...
export function AudioUpload({
  ...
  }, [audioFiles.length, existingAudioFiles.length, maxFiles, disabled, toast]);

  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    onDrop,
    accept: {
      'audio/*': SUPPORTED_AUDIO_FORMATS.map(format => 
        format.replace('audio/', '.')
      )
    },
    maxSize: AUDIO_CONSTRAINTS.maxFileSize,
    disabled,
    multiple: true
  });
              ...
              <div key={audioFile.id} className="space-y-4 p-4 border rounded-lg">
                ...
                {audioFile.status === 'completed' && audioFile.result && (
                  <div className="space-y-2 text-sm text-green-600">
                    ...
                    {audioFile.result.compressedFile && (
                      <p className="text-xs text-gray-500">
                        Compressed from {formatFileSize(audioFile.result.originalFile.size)} to{' '}
                        {formatFileSize(audioFile.result.compressedFile.size)} 
                        ({Math.round((1 - audioFile.result.compressedFile.size / audioFile.result.originalFile.size) * 100)}% reduction)
                      </p>
                    )}
                  </div>
                )}
